...
const Books = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) =>
        totalwishdispatch({ type: "WISHLIST", payload: res.data })
      );
  }, []);
  useEffect(() => {
    axios
  ...
  const handlewishlist = (id, name, author, url) => {
    ...
    if (totalwishstate != undefined) {
      if (totalwishstate.length != 0) {
        const checkpres = totalwishstate.find((item) => item.bid === id);
  ...
  const handleissue = (id, rent, name, author, url) => {
    ...
    if (totalissuestate != undefined) {
      if (totalissuestate.length != 0) {
        const checkpres = totalissuestate.find((item) => item.bid == id);
        console.log(checkpres);
        if (checkpres != undefined) {
          if (checkpres.bid == id) {
            setDays(0);
    ...
    axios
      .post(`http://localhost:5000/issue/${id}`, {
        email: emailstate,
        role: rolestate,
        period: days,
        amount: days * rent,
      ...
      {!load ? (
        <>
          <div className="loader">
            <ReactLoading width="10em" height="10em" type={"spinningBubbles"} color="#1D1A54" />
          </div>
        </>
      ) : (
        <>
        <div className="mainn">
          <div className="main-heading">Books</div>
                    ...
                    return (
                      <div className="main_displayy" key={index}>
                        <div className="card_component">
                          ...
                          <div className="card_content">{book.desc}</div>
                          <button
                            onClick={() =>
                              handlewishlist(
                                book.bid,
